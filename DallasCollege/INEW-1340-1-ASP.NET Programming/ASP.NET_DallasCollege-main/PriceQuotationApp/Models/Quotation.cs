...
namespace PriceQuotationApp.Models
{
    public class Quotation
    {
        ...
        // ? in ASP.NET means the data type is nullable. 
        public decimal? DiscountPercent { get; set; }


        [Required]
        [Range(0.0, 15.0, ErrorMessage = "Tax percent must be between 0 and 15")]
        ...
        public decimal CalculateTotal()
        {
            if (Subtotal.HasValue)
            {
                ...
                decimal subtotalAfterDiscount = Subtotal.Value - discount;
                
                // Calculate tax properly
                decimal tax = 0m;
                if (TaxRate.HasValue)
                {
                    tax = subtotalAfterDiscount * (TaxRate.Value / 100);
